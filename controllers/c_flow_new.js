'use strict';
const fs = require('fs');
const upload = require('../dao/upload.js');
...
module.exports = function (router) {
    ...
    router.post('/upload', async (ctx, next) => {
        ...
        for (var i=0; i<files.length; i++){
            ...
            let id = await upload.insert(flow_list_id, file_name, file_path, fields['another_temp_id'], next);
            console.log('=========');
            console.log(id);
            result.push({
            id: id,
    ...
    if( flow_list_id == 0 ) {
        ...
        var attachments = await upload.find_by_fli(flow_list_id, next);
    ...
    let attachment = await upload.delete_by_id(id, next);
    ...
    var flow_list_id = ctx.params.flow_list_id;
    let attachments = await upload.delete_by_fli(flow_list_id, next);
    ...
    let attachments = await upload.delete_by_ti(temp_id, next);
    if ( attachments.success != undefined ) {
        for(let attachment of attachments.success){
        fs.unlinkSync(attachment.file_path);
        }
    ...
    console.log(id);
    let attachment = await upload.find_by_id(id, next);
