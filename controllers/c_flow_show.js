...
let productDistribution  = async(affair, next) => {
    // 产品发行流程
    let res;
    if(affair.flow_id == 'b395b7615f9811e6b480b888e3e688de'){
...
module.exports = function (router) {
    // 合同审批流程
    router.get('/get_attatable_task/:task_id', async (ctx, next) => {
        let task_id = ctx.params.task_id;
        let affair = await d_flow.find_task(task_id, next);
        let res = await contractApproval(affair, next);
        ctx.response.body = res;
    });
    router.get('/get_attatable_affa/:affa_id', async (ctx, next) => {
    ...
    // 产品发行流程
    router.get('/get_fxtable_task/:task_id', async (ctx, next) => {
        let task_id = ctx.params.task_id;
        let affair = await d_flow.find_task(task_id, next);
        let res = await productDistribution(affair, next);
        ctx.response.body = res;
    });
    router.get('/get_fxtable_affa/:affa_id', async (ctx, next) => {
        let affa_id = ctx.params.affa_id;
        let affair = await d_flow.find_affar(affa_id, next);
        let res = await productDistribution(affair, next);
        ctx.response.body = res;
